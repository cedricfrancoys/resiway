...
$params = QNLib::announce(	
	array(	
    'description'	=>	"Registers a vote up performed by a user on given comment of a document",
    'params' 		=>	array(                                         
                        'comment_id'	=> array(
                                            'description'   => 'Identifier of the document comment the user votes up.',
                                            'type'          => 'integer', 
                                            'required'      => true
                                            )                                       
                        )
...
list($action_name, $object_class, $object_id) = [
    'resilib_documentcomment_voteup', 
    ...
    $result = ResiAPI::performAction(
        ...
        [                                                           // $limitations
            ...
            function ($om, $user_id, $action_id, $object_class, $object_id) {
                ...
                if($res > 0 && count($res) > RESILIB_COMMENT_VOTEUP_DAILY_MAX) {
