...
try {
    ...
    $documents_ids = $om->search('resilib\Document');
    if($documents_ids > 0 && count($documents_ids)){
        $documents = $om->read('resilib\Document', $documents_ids, ['id', 'title_url', 'title']);
        foreach($documents as $document_id => $document) {
            switch($params['output']) {
            case 'txt':
                echo "https://www.resiway.org/document/{$document['id']}/{$document['title_url']}".PHP_EOL;
                break;
            case 'html':
                echo '<a href="https://www.resiway.org/document/'.$document['id'].'/'.$document['title_url'].'">'.$document['title'].'</a><br />'.PHP_EOL;
            }
        }
