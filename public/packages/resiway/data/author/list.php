...
$params = QNLib::announce(	
	array(	
    'description'	=>	"Returns a list of authors objects matching the received criteria",
    'params' 		=>	array(                                         
                        'domain'		=> array(
...
try {
    ...
    if($params['total'] < 0) {        
        $ids = $om->search('resiway\Author', $params['domain'], $params['order'], $params['sort']);
        if($ids < 0) throw new Exception("request_failed", QN_ERROR_UNKNOWN);
        $total = count($ids);
		$authors_ids = array_slice($ids, $params['start'], $params['limit']);
    }
    else {
        $authors_ids = $om->search('resiway\Author', $params['domain'], $params['order'], $params['sort'], $params['start'], $params['limit']);
        if($authors_ids < 0) throw new Exception("request_failed", QN_ERROR_UNKNOWN);
    }
