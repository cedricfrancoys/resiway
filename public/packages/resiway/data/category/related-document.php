...
try {
    ...
    $domain = QNLib::domain_condition_add($domain, ['count_documents','>', 0]);    
    ...
    $categories_ids = $om->search('resiway\Category', $domain, 'count_documents', 'desc', 0, $params['limit']);
    
    $categories_ids = array_diff($categories_ids, [$object_id]);
    
    if(!empty($categories_ids)) {    
        // retrieve categories
        $res = $om->read('resiway\Category', $categories_ids, ['id', 'title', 'title_url', 'path', 'count_documents']);
