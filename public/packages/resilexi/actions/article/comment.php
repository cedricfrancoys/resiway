...
$params = QNLib::announce([
    'description'	=>	"Adds a new comment to a article",
    'params' 		=>	[
        'article_id'	=> array(
                            'description'   => 'Identifier of the article the user is commenting.',
                            'type'          => 'integer', 
                            'required'      => true
                            ),
        'content'	    => array(
...
try {
    ...
    $result = ResiAPI::performAction(
        ...
        use ($params) {    
            ...
            $comment_id = $om->create('resilexi\ArticleComment', [ 
                            'creator'       => $user_id,     
            ...
            $actionlog_id = $om->create('resiway\ActionLog', [
                            'user_id'               => $user_id,
                            'author_id'             => $user_id,
                            'action_id'             => ResiAPI::actionId('resilexi_articlecomment_post'), 
                            'object_class'          => 'resilexi\ArticleComment', 
            ...
            $res = $om->read('resilexi\ArticleComment', $comment_id, ['creator', 'created', 'content', 'score']);
            if($res > 0) {
                $result = array(
                            'id'        => $comment_id,
        ...
        [                                                          // $limitations
            ...
            function ($om, $user_id, $action_id, $object_class, $object_id) {
                ...
                if($res > 0 && count($res) > RESILIB_COMMENTS_DAILY_MAX) {
