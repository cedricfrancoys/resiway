...
$params = QNLib::announce(	
	array(	
    'description'	=>	"Registers a vote up performed by a user on given comment of a article",
    'params' 		=>	array(                                         
                        'comment_id'	=> array(
                                            'description'   => 'Identifier of the article comment the user votes up.',
                                            'type'          => 'integer', 
                                            'required'      => true
                                            )                                       
                        )
...
list($action_name, $object_class, $object_id) = [
    'resilexi_articlecomment_voteup', 
    ...
    $result = ResiAPI::performAction(
        ...
        [                                                           // $limitations
            ...
            function ($om, $user_id, $action_id, $object_class, $object_id) {
                ...
                if($res > 0 && count($res) > RESILEXI_COMMENT_VOTEUP_DAILY_MAX) {
