...
if(file_exists("../cache/sqldesign_{$params['package']}.xml")) {
    $result = file_get_contents("../cache/sqldesign_{$params['package']}.xml");
...
$result = $xml->asXML();
file_put_contents("../cache/sqldesign_{$params['package']}.xml", $result);
header('Access-Control-Allow-Origin: *');
header('Content-type: text/xml; charset: utf-8');
header('Content-Length: '.strlen($result));
print($result);